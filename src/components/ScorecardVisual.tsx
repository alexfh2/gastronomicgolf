...
// Standard par for a typical 18-hole course (par 72)
// This could be made configurable per course in the future
...
const ScorecardVisual: React.FC<ScorecardVisualProps> = ({ scores, par = DEFAULT_PAR }) => {
  ...
  const renderScore = (score: number, holePar: number) => {
    ...
    // Double bogey+: filled darker square
    return (
  ...
  const renderHalf = (halfScores: number[], halfPar: number[], startHole: number, total: number) => (
  ...
  return (
    <div className="space-y-2">
      {renderHalf(front9, frontPar, 1, frontTotal)}
      {renderHalf(back9, backPar, 10, backTotal)}
